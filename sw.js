...
.portalStack{width:42px;height:42px;border:1px solid rgba(255,255,255,.22);border-radius:12px;background:rgba(255,255,255,.14);color:#fff;text-decoration:none;display:flex;flex-direction:column;align-items:center;justify-content:center;line-height:1}
...
.memberAvatar{width:48px;height:48px;border-radius:15px;background:rgba(255,255,255,.16);display:grid;place-items:center;font-size:17px;font-weight:900}
.memberMain{min-width:0}.memberMain small{font-size:8px!important;letter-spacing:.08em;opacity:.62!important;font-weight:800}.memberMain h2{font-size:18px!important;margin:3px 0 0!important;line-height:1.35}.memberMeta{font-size:9px;margin-top:5px;opacity:.72;font-weight:700}
.memberNext{grid-column:1/-1;margin-top:1px;padding-top:10px;border-top:1px solid rgba(255,255,255,.13);display:flex;align-items:center;gap:10px;min-width:0;cursor:pointer}.memberNextLabel{font-size:8px;font-weight:900;letter-spacing:.12em;opacity:.58;flex:none}.memberNextText{font-size:10px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1}.memberNextArrow{font-size:16px;opacity:.6}
    ...
    const dashScript=`<script>(()=>{const enhance=()=>{const h=document.querySelector('.hello');if(!h||h.classList.contains('memberDash'))return;h.classList.add('memberDash');const name=document.getElementById('helloName');const date=document.getElementById('todayText');const avatar=document.createElement('div');avatar.className='memberAvatar';avatar.textContent='8';const main=document.createElement('div');main.className='memberMain';if(date)date.textContent='8LABO ACADEMY MEMBER';if(name)main.append(date,name);const meta=document.createElement('div');meta.className='memberMeta';meta.textContent='ACADEMY 会員ページ';main.appendChild(meta);h.prepend(main);h.prepend(avatar);const next=document.createElement('div');next.className='memberNext';next.innerHTML='<span class="memberNextLabel">NEXT</span><span class="memberNextText">次回予定を確認</span><span class="memberNextArrow">›</span>';next.onclick=()=>{const b=[...document.querySelectorAll('.bottom button')].find(x=>x.textContent.includes('予定'));if(b)b.click()};h.appendChild(next);const sync=()=>{const c=document.getElementById('nextCard');if(c&&c.innerText.trim())next.querySelector('.memberNextText').textContent=c.innerText.trim().replace(/\\s+/g,' ')};sync();const c=document.getElementById('nextCard');if(c)new MutationObserver(sync).observe(c,{childList:true,subtree:true,characterData:true})};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',enhance);else enhance();})();</script>`;
